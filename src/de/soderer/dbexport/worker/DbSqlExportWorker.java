...
import de.soderer.utilities.plugin.SqlUtil;
...
public class DbSqlExportWorker extends AbstractDbExportWorker {
	...
	protected void startOutput(final Connection connection, final String sqlStatement, final List<String> columnNames) throws Exception {
		...
		//打印导出日志@20230704 by huangzh
		System.out.println("fileWriter sqlStatement-->"+sqlStatement);
		if (sqlStatement.toUpperCase().startsWith("SELECT * FROM ")) {
			...
			//add by huangzh@20230608
			List<String> tableNames = SqlUtil.getTableList(sqlStatement);
//			for (int i = 0; i < tableNames.size(); i++) {
//				System.out.println("Current Table --> "+tableNames.get(i));
//			}
	...
	protected void writeColumn(final String columnName, final Object value) throws Exception {
		...
		if (value == null) {
			...
			values.add(getBigDecimal(value).stripTrailingZeros().toPlainString());
		} else {
